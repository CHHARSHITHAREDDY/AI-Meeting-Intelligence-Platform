(function () {
  ...
  function appendTranscriptLine(speaker, text, isInterim = false) {
    ...
    } else if (lower.includes('?') || lower.startsWith('how') || lower.startsWith('what') || lower.startsWith('why') || lower.startsWith('who') || lower.startsWith('when') || lower.startsWith('can we')) {
      addInsight('insight-question', 'QUESTION', text);
    }
  }
  ...
  // 8. Export & Workflow Toolbar Handlers
  ...
  function getSessionInsightsData() {
    const lines = Array.from(transcriptFeed.querySelectorAll('.transcript-line:not(.interim-line)'))
      .map(el => el.textContent.trim())
      .filter(txt => txt && !txt.includes('Click ▶ Start') && !txt.includes('Recording active'));

    const decisions = Array.from(shadow.querySelectorAll('.insight-decision')).map(el => el.textContent.replace(/^\[DECISION\]\s*/, '').trim());
    const tasks = Array.from(shadow.querySelectorAll('.insight-task')).map(el => el.textContent.replace(/^\[TASK\]\s*/, '').trim());
    const risks = Array.from(shadow.querySelectorAll('.insight-risk')).map(el => el.textContent.replace(/^\[RISK\]\s*/, '').trim());
    const questions = Array.from(shadow.querySelectorAll('.insight-question')).map(el => el.textContent.replace(/^\[QUESTION\]\s*/, '').trim());

    return { lines, decisions, tasks, risks, questions };
  }

  expEmailBtn.addEventListener('click', () => {
    const { lines, decisions, tasks, risks, questions } = getSessionInsightsData();
    if (lines.length === 0) {
      alert('No meeting data recorded yet to export.');
      return;
    }
...
*:heavy_check_mark: Action Items (${tasks.length}):*
${tasks.map(t => `> • ${t}`).join('\n') || '> _None_'}

*:warning: Risks Identified (${risks.length}):*
${risks.map(r => `> • ${r}`).join('\n') || '> _None_'}

*:question: Open Questions (${questions.length}):*
${questions.map(q => `> • ${q}`).join('\n') || '> _None_'}`;
  ...
  expJiraBtn.addEventListener('click', () => {
    const { tasks } = getSessionInsightsData();
    if (tasks.length === 0) {
      alert('No action items detected to export.');
      return;
    }

    const jiraContent = tasks.map((t, idx) => `[TASK-${idx + 1}] Summary: ${t} | Priority: Medium | Status: To Do | Created: ${new Date().toISOString().split('T')[0]}`).join('\n');
  ...
  expStatsBtn.addEventListener('click', () => {
    const { lines, decisions, tasks, risks } = getSessionInsightsData();
    if (lines.length === 0) {
      alert('No transcript lines recorded yet for speaker analysis.');
      return;
    }
    ...
    lines.forEach(line => {
      ...
      const speaker = match ? match[1].trim() : 'Participant';
    ...
    const stats = Array.from(speakerMap.entries())
      .map(([name, count]) => ({
        name,
        pct: totalWords > 0 ? Math.round((count / totalWords) * 100) : 0
      }))
      .sort((a, b) => b.pct - a.pct);

    const score = Math.min(100, Math.max(50, Math.round(70 + decisions.length * 8 + tasks.length * 5 - risks.length * 3)));

    let statsCard = insightList.querySelector('.insight-stats');
    if (!statsCard) {
      if (insightList.children[0]?.textContent.includes('Listening')) {
        insightList.innerHTML = '';
      }
      statsCard = document.createElement('div');
      statsCard.className = 'insight-item insight-stats';
      statsCard.style.background = 'rgba(99, 102, 241, 0.15)';
      statsCard.style.borderLeft = '3px solid #6366F1';
      statsCard.style.color = '#e0e7ff';
      statsCard.style.marginBottom = '6px';
      insightList.prepend(statsCard);
    }

    const breakdownHtml = stats.map(s => `<div><strong>${s.name}</strong>: ${s.pct}% talk time</div>`).join('');
